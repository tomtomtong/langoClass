...
function readSettings() {
  ...
  try {
    ...
    const azureApiKey = typeof parsed.azureApiKey === "string" ? parsed.azureApiKey.trim() : "";
    const azureEndpoint =
      typeof parsed.azureEndpoint === "string" ? parsed.azureEndpoint.trim().replace(/\/$/, "") : "";
    const azureDeployment =
      typeof parsed.azureDeployment === "string" ? parsed.azureDeployment.trim() : "";
    const azureApiVersion =
      typeof parsed.azureApiVersion === "string" ? parsed.azureApiVersion.trim() : "";
...
function writeSettings(partial) {
  ...
  if (typeof next.azureApiKey === "string") {
    next.azureApiKey = next.azureApiKey.trim();
  }

  if (typeof next.azureEndpoint === "string") {
    next.azureEndpoint = next.azureEndpoint.trim().replace(/\/$/, "");
  }

  if (typeof next.azureDeployment === "string") {
    next.azureDeployment = next.azureDeployment.trim();
  }

  if (typeof next.azureApiVersion === "string") {
    next.azureApiVersion = next.azureApiVersion.trim();
  }
