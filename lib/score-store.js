...
const DATA_FILE = paths.studentScoresFile;
...
function clearAll() {
  ...
  writeStore({ students: {}, records: [] });
  return cleared;
}

function deleteRecordsForTeacherCourses(teacherId, courseIds) {
  const tid = Number(teacherId);
  ...
  store.records = store.records.filter((record) => {
    if (record.teacherId === tid && idSet.has(Number(record.courseId))) {
      affectedStudents.add(studentKey(record.teacherId, record.classId, record.studentUserId));
      return false;
    }
    return true;
  });

  for (const key of affectedStudents) {
    const student = store.students[key];
    if (!student) continue;
    const [t, c, u] = key.split(":");
    const computed = totalsFromRecords(store, t, c, u);
    store.students[key] = {
      ...student,
      totalScore: computed.totalScore,
      exerciseCount: computed.exerciseCount,
...
module.exports = {
  ...
  getStats,
  clearAll,
  deleteRecordsForTeacherCourses,
  studentKey,
  recordKey,
};
