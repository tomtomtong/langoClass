...
async function extractPdf(buffer, services = {}) {
  ...
  if (services.hasVisionApi === false) {
    throw new Error(
      `No readable text found in ${services.filename || "this PDF"}. This looks like a scanned/image PDF — save your OpenRouter API key in Config → OpenRouter (a vision model alone does not authenticate), then try again.`
    );
  }
