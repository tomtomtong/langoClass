const { parseJsonFromLlm } = require("./exercise-generator");
...
function buildCoursePlanPrompt({ material, format, candidates, langCode }) {
  ...
  return [
    {
      ...
      content:
        ...
        "The teacher already chose an exercise format. Copy that format onto every section. " +
        "Do not add, remove, or change exercise types, counts, or difficulty. " +
        "Return ONLY valid JSON (no markdown fences, no commentary).",
    },
    {
      role: "user",
      content:
        `Learner language/context: ${langCode || "en"}.\n` +
        `LOCKED FORMAT (copy onto every section): ${JSON.stringify(format.types)} at ${format.difficulty} difficulty.\n` +
        ...
        `CANDIDATE SECTIONS:\n${candidateList}\n\n` +
        `FULL MATERIAL (may be truncated):\n${String(material || "").slice(0, 12000)}`,
    },
  ];
}
...
async function analyzeCourseMaterial(options, llmComplete) {
  ...
  if (!Object.keys(types).length) {
    throw new Error("Pick a format with at least one exercise type first.");
  }
  ...
  const clipped =
    material.length > MAX_PLAN_MATERIAL_CHARS
      ? `${material.slice(0, MAX_PLAN_MATERIAL_CHARS).trim()}\n\n[…truncated for planning]`
      : material;
