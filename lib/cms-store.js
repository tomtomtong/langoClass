...
function createCourse(teacherId, payload) {
  ...
  const course = {
    ...
    sourceCommunityId:
      payload.sourceCommunityId != null ? Number(payload.sourceCommunityId) || null : null,
...
function updateCourse(courseId, teacherId, payload) {
  ...
  if (payload.sourceCommunityId !== undefined) {
    course.sourceCommunityId =
      payload.sourceCommunityId != null ? Number(payload.sourceCommunityId) || null : null;
  }
  if (payload.communityListingId !== undefined) {
    course.communityListingId =
      payload.communityListingId != null ? Number(payload.communityListingId) || null : null;
  }
  if (payload.communityPublishedAt !== undefined) {
    course.communityPublishedAt = payload.communityPublishedAt || null;
  }
  if (payload.communityFeatured !== undefined) {
    course.communityFeatured = Boolean(payload.communityFeatured);
  }
  if (payload.level != null) course.level = String(payload.level).trim().slice(0, 40);
  if (payload.estimatedMinutes !== undefined) {
    const mins = Number(payload.estimatedMinutes);
    course.estimatedMinutes = Number.isFinite(mins) && mins > 0 ? mins : null;
  }
  if (payload.objectives != null) {
    course.objectives = Array.isArray(payload.objectives)
      ? payload.objectives.map((line) => String(line).trim()).filter(Boolean).slice(0, 8)
      : [];
  }
  if (payload.authorNotes != null) {
    course.authorNotes = String(payload.authorNotes).trim().slice(0, 500);
  }
  if (payload.plannedStart != null) course.plannedStart = String(payload.plannedStart).trim().slice(0, 32);
  if (payload.plannedEnd != null) course.plannedEnd = String(payload.plannedEnd).trim().slice(0, 32);
...
function normalizeExercise(raw, order) {
  ...
  const speakLangCode = String(raw.speakLangCode || "")
    .trim()
    .toLowerCase()
    .split(/[-_]/)[0]
    .slice(0, 8);
  if (speakLangCode) base.speakLangCode = speakLangCode;
  ...
  if (type === "video") {
    ...
    const legacyQuestions = collectBuzzinQuestionsFromRaw(raw);
    const fallbackStt = String(raw.sttLanguage || "")
      .trim()
      .toLowerCase()
      .split("-")[0]
      .slice(0, 8);
    ...
    let items = rawItems
      .map((item) => {
        const topic = String(item?.topic || item?.title || item?.question || item?.text || "")
          .trim()
          .slice(0, 500);
        const correctAnswer = String(
          ...
          .trim()
          .slice(0, 500);
        const sttLanguage = String(item?.sttLanguage || fallbackStt || "")
          .trim()
          .toLowerCase()
          .split("-")[0]
          .slice(0, 8);
    ...
    if (!items.length && legacyQuestions.length) {
      items = legacyQuestions.map((topic) => ({
        topic,
        ...(fallbackStt ? { sttLanguage: fallbackStt } : {}),
      }));
    }

    if (!items.length) {
      const topic = String(raw.title || "")
        .trim()
        .slice(0, 500);
      if (topic) {
        items = [{ topic, ...(fallbackStt ? { sttLanguage: fallbackStt } : {}) }];
      }
    }

    base.subTitle = base.subTitle || "Buzz In";
    base.items = items.slice(0, MAX_BUZZIN_TOPICS);
  } else {
    const isFastMc = type === "fastmcquiz";
    base.type = isFastMc ? "fastmcquiz" : "mcquiz";
    base.subTitle = base.subTitle || (isFastMc ? "Lightning round" : "MC Quiz");
...
function courseDetailResponse(course) {
  ...
  return {
    ...
    course: {
      ...
      sourceCommunityId: normalized.sourceCommunityId || null,
      communityListingId: normalized.communityListingId || null,
      communityPublishedAt: normalized.communityPublishedAt || null,
      communityFeatured: Boolean(normalized.communityFeatured),
    },
