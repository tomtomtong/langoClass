...
const DATA_FILE = path.join(DATA_DIR, "teacher-courses.json");
...
function normalizeCourse(course) {
  ...
  course.sections = (course.sections || []).map((section, i) => normalizeSection(section, i + 1));
  return course;
}
...
function normalizeExercise(raw, order) {
  ...
  if (type === "video") {
    const url = String(raw.items?.[0]?.videoUrl || raw.videoUrl || "").trim().slice(0, 500);
    ...
    base.items = [
      {
        title: String(item.title || base.title).trim().slice(0, 200),
        buddy: normalizeBuzzinBuddyField(item.buddy),
        questions: collectBuzzinQuestionsFromRaw(raw).slice(0, 20),
      },
    ];
  } else {
...
function normalizeSection(raw, order) {
  const exercises = (raw.exercises || []).map((exercise, i) => normalizeExercise(exercise, i + 1));
  return {
    id: raw.id,
