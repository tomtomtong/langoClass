...
function markParticipantOfflineBySocket(session, socketId) {
...
function markParticipantOnline(session, userId, socketId) {
  const participant = session.participants.get(userId);
  if (!participant) return null;
  if (socketId) participant.socketId = socketId;
  participant.online = true;
  participant.offlineSince = null;
  if (participant.removalTimer) {
    clearTimeout(participant.removalTimer);
    participant.removalTimer = null;
  }
  return participant;
}

function removeParticipant(session, userId) {
  const participant = session.participants.get(userId);
  if (!participant) return null;
  if (participant.removalTimer) {
    clearTimeout(participant.removalTimer);
...
module.exports = {
  ...
  participantPayload,
  addParticipant,
  markParticipantOfflineBySocket,
