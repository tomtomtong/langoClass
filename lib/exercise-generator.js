  ...
  return [
    {
      ...
      content:
        ...
        "For mcquiz/fastmcquiz items: {title, options:[{text,isCorrect}], timeLimit, imageRef:null}. " +
        "For buzzin items: {topic, correctAnswer, imageRef:null}. " +
        "When IMAGE ASSET CATALOG is provided, pick the best matching imageRef for questions about photos/diagrams/objects. " +
        "Use null when no asset fits. Never invent imageRef ids. " +
...
function normalizeMcItem(item, { fast = false } = {}) {
  ...
  return {
    ...
    imageRef: item?.imageRef ? String(item.imageRef).trim() : null,
  };
}
  ...
  return [
    {
      ...
      content:
        ...
        "For mcquiz/fastmcquiz items: {title, options:[{text,isCorrect}], timeLimit, imageRef:null}. " +
        "For buzzin items: {topic, correctAnswer, imageRef:null}. " +
        "When IMAGE ASSET CATALOG is provided, you may set imageRef on items that need a picture. " +
        "Every mc option set must have exactly one isCorrect:true. " +
