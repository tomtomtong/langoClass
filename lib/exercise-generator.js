...
const cantoneseSpeechText = require("./cantonese-speech-text");
...
function languageName(code) {
  ...
  const question = CMS_QUESTION_LANGUAGES.find((entry) => entry.code.toLowerCase() === lower);
...
function languageScriptHint(code) {
  ...
  if (normalized === "yue") {
    return (
      "Use Traditional Chinese characters (繁體中文) with natural spoken Hong Kong Cantonese (廣東話口語), NOT Mandarin (普通話). " +
      "Never use Simplified Chinese (简体中文). " +
      "Prefer Cantonese particles and words: 咩/什麼, 點樣, 呢個, 嗰個, 同, 冇, 喺, 嘅, 佢, 哋, 睇, 講. " +
      "Avoid Mandarin-only words: 什么, 这个, 那个, 怎样, 通过, 进行, 以及, 并且, 描述, 他们, 我们. " +
      'Example stem: "嗰啲街仔遊戲主要係邊個年代？" Example option: "1960年代". Example buzz-in topic: "講吓你細個最鍾意玩嘅街仔遊戲。"'
    );
  }
  if (normalized === "zh") {
    return (
...
function textMatchesTargetLanguage(text, langCode) {
  ...
  if (code === "zh" || code === "yue") {
    ...
    if (code === "yue" && cantoneseSpeechText.containsMandarinWording(sample)) return false;
    return true;
  }
  if (code === "ja") {
...
function buildLanguageCorrectionPrompt(langCode, exercises) {
  ...
  const traditionalNote =
    code === "zh" || code === "yue"
      ...
      : " ";
  const cantoneseNote =
    code === "yue" ? ` ${cantoneseSpeechText.cantoneseSpeechPromptHint()} ` : " ";
  return (
  ...
  return [
    ...
    },
    {
      ...
      content:
        ...
        `Generate:\n${requests.join("\n")}\n\n` +
        `Use these subTitle values: mcquiz → "MC Quiz", fastmcquiz → "Lightning round", buzzin → "Buzz In".\n` +
        ...
        try {
          fs.appendFileSync(
            ...
            `${JSON.stringify({
              ...
              runId: "cantonese-gen",
              hypothesisId: "H1",
              location: "exercise-generator.js:generateExercisesFromMaterial",
              message: "Cantonese generation attempt",
              data: {
                attempt: attempt + 1,
                wrongLanguage,
                badSamples: wrongLanguage
                  ? collectWrongLanguageSamples(validated.exercises, langCode)
                  : [],
                sampleOut: collectExerciseTexts(validated.exercises).slice(0, 2),
              },
              timestamp: Date.now(),
            })}\n`
          );
...
function parseAddQuestionIntent(revision) {
  ...
  const patterns = [
    { re: /add\s+(\d+)\s+(?:lightning(?:\s*round)?|fast\s*mc(?:\s*quiz)?|fastmcquiz)/i, type: "fastmcquiz" },
...
function parseQuestionNumberFromText(text) {
  const sample = String(text || "");
  const questionMatch = sample.match(/question\s*(\d+)/i);
  if (questionMatch) return Math.max(1, Number.parseInt(questionMatch[1], 10) || 1);
  const qMatch = sample.match(/\bq\s*(\d+)\b/i);
  if (qMatch) return Math.max(1, Number.parseInt(qMatch[1], 10) || 1);
  const cnMatch = sample.match(/問題\s*(\d+)/);
  if (cnMatch) return Math.max(1, Number.parseInt(cnMatch[1], 10) || 1);
  return null;
}

function parseRegenerateQuestionIntent(revision, contextQuestionNumber) {
  ...
  if (parseConvertQuestionIntent(revision, contextQuestionNumber)) return null;
  const questionNumber =
    parseQuestionNumberFromText(text) || validContextQuestionNumber(contextQuestionNumber);
...
async function regenerateItemWithLlm(options, llmComplete) {
  const {
    sourceType,
    item,
    material,
  ...
  const avoidBlock = avoidTitles.length
    ? `Do NOT reuse these existing question stems/topics (create a clearly different question):\n${avoidTitles.map((title) => `- ${title}`).join("\n")}\n\n`
    : "";
  const messages = [
    {
      role: "system",
      content:
        "Generate ONE new classroom exercise item to replace an existing question. " +
        "Use the same source material and learning goals, but write a DIFFERENT question (new angle, wording, or detail). " +
        languageOutputRequirement(langCode) +
        " Return ONLY valid JSON for the single new item (no markdown). " +
        (sourceType === "buzzin"
          ? 'Schema: {"topic":"string","correctAnswer":"string"}'
          : 'Schema: {"title":"string","options":[{"text":"string","isCorrect":boolean}],"timeLimit":number,"image":null}') +
        ". Exactly one option must have isCorrect:true for mc types.",
    },
    {
      ...
      content:
        `${languageOutputRequirement(langCode)}\n` +
        `Replace one existing ${TYPE_LABELS[sourceType] || sourceType} question with a NEW different question.\n` +
        `Target language: ${languageName(langCode)}.\n` +
        `${difficultyGuidance(difficulty)}\n\n` +
        formatTeacherInstructions(instructions) +
        avoidBlock +
  ...
  const parsed = parseJsonFromLlm(reply);
  const raw = parsed?.item ?? parsed;
  if (sourceType === "buzzin") {
    const normalized = normalizeBuzzinItem(raw);
    if (!normalized) throw new Error("Could not regenerate Buzz In question.");
    return normalized;
  }
  const normalized = normalizeMcItem(raw, { fast: sourceType === "fastmcquiz" });
  if (!normalized) throw new Error("Could not regenerate quiz question.");
  return normalized;
}

function itemPrimaryText(type, item) {
  if (!item) return "";
  return String(type === "buzzin" ? item.topic : item.title || "").trim();
}

async function regenerateQuestionInDraft(options, llmComplete) {
  const { regenIntent, editable, videos, material, model, apiKey } = options;
  const { questionNumber } = regenIntent;
  ...
  const previousText = itemPrimaryText(target.type, target.item);
  const avoidTitles = flat
    .map((entry) => itemPrimaryText(entry.type, entry.item))
    .filter(Boolean);

  let newItem = null;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    newItem = await regenerateItemWithLlm(
      {
    ...
    const nextText = itemPrimaryText(target.type, newItem);
    if (!previousText || !nextText || nextText !== previousText) break;
  }
  ...
  try {
    fs.appendFileSync(
      ...
      `${JSON.stringify({
        ...
        location: "exercise-generator.js:regenerateQuestionInDraft",
        message: "Single question regen complete",
        data: {
          questionNumber,
          previousText: previousText.slice(0, 120),
          nextText: nextText.slice(0, 120),
  ...
  return {
    exercises,
    summary: `Regenerated question ${questionNumber}.`,
    model,
    revisionMode: "regen-in-place",
    stats: {
      questionNumber,
  ...
  return [
    {
      ...
      content:
        ...
        "When the teacher specifies a number (e.g. 'add 1 Lightning round'), add exactly that many question items total—not a full exercise with multiple questions. " +
        "'Add 1 Lightning round' means one fastmcquiz question item. " +
...
async function reviseExercisesFromDraft(options, llmComplete) {
  ...
  const regenIntent = parseRegenerateQuestionIntent(revision, options.questionNumber);
  if (regenIntent) {
    return regenerateQuestionInDraft(
      { ...options, revision, editable, videos, regenIntent, material, model },
      llmComplete
    );
  }
