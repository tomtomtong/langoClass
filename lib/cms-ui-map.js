...
const CMS_SCREENS = {
  ...
  home: {
    ...
    actions: [
      ...
      {
        label: "Live class insights",
        type: "card button",
        result: "Opens Live class insights (recent Host sessions and assigned courses).",
      },
    ],
  },
  dashboard: {
    heading: "Live class insights",
    purpose: "Recent Host sessions and assigned course progress for the selected class.",
    actions: [
      {
        label: "Class",
        type: "dropdown",
  ...
  edit: {
    ...
    tabs: [
      {
        label: "1 · Details",
        id: "details",
        fields: ["Course name", "Description", "Cover image", "Who can host this"],
        actions: [
          { label: "Save details", type: "primary button", note: "Stays on Details and prompts the teacher to continue to Sections." },
          { label: "Continue to Sections", type: "secondary button" },
          { label: "Export", type: "secondary button", note: "Shown only when editing an existing course, not during New course." },
          {
            label: "Share to Community",
            type: "secondary button",
            note: "Publishes a public snapshot. Becomes Update Community listing after sharing. Hidden for New course and for courses copied from Community.",
          },
          { label: "Unshare", type: "secondary button", note: "Removes the public listing. Hidden until the course is shared, and during New course." },
          { label: "Delete course", type: "text danger", note: "Quiet control in the secondary row. Hidden during New course." },
        ],
      },
      {
        label: "Sections",
        ...
        actions: [
          { label: "Add section", type: "button", location: "Bottom of the section list, or Start a section → Add section manually." },
        ...
        startChooser: [
          { label: "Generate one section", purpose: "Creates a blank section and opens the AI wizard for it." },
          { label: "Build full course from document", purpose: "Split one document into multiple sections, then generate." },
          { label: "Add section manually", purpose: "Opens a new section with a blank exercise ready to type." },
        ],
        batchAi: {
    ...
    exercisesSubview: {
      ...
      aiPanel: {
        ...
        entry: "Generate for this section — opens the section wizard. Change approach returns to the Sections start chooser.",
        wizardSteps: ["Format", "Material", "Review", "Publish"],
        wizardNav: ["Back", "Continue", "Generate exercises", "Continue to publish", "Publish to section"],
        reviewHint: "Use Ask Lango (bottom-right) to revise generated questions on Review.",
        coursePath: {
          label: "Build full course from document",
          wizardSteps: ["Material", "Outline", "Review", "Publish"],
          wizardNav: [
            "Build outline",
            "Generate N sections",
            "Confirm generate",
            "Continue to publish",
            "Publish course (create / add to existing / skip)",
          ],
        },
      },
    },
...
const CMS_FLOWS = {
  createCourse: {
    ...
    steps: [
      ...
      'You land on **New course** → **1 · Details**. Enter **Course name** and **Description**. Export, Share, and Delete are hidden until you open the course from My courses later.',
      'Click **Save details** (you stay on this course), then **Continue to Sections**.',
      'Open the **Sections** tab. New courses have no default Section 1 — pick **Generate one section**, **Build full course from document**, or **Add section manually**.',
      'To fill an existing section, click **Edit** on that card.',
    ],
  },
  generateExercises: {
    title: "Generate exercises with AI (one section)",
    steps: [
      'Open a course → **Sections** tab. Choose **Generate one section**, **Build full course from document**, or **Add section manually** before opening a section.',
      'Inside a section, click **Generate for this section** for the section wizard, or **Change approach** to return to the Sections chooser.',
      'Section path: **Format** → **Material** → **Review** → **Publish**.',
      'Course path: **Material** → **Outline** (approve split + compact play format) → **Generate** then **Confirm generate** → **Review** → **Publish** (create, add to existing, or skip).',
  ...
  shareCommunity: {
    ...
    steps: [
      ...
      "On the **Details** tab, click **Share to Community**. This publishes a snapshot — later edits do not update the listing until you click **Update Community listing**.",
...
function getUiGuidance(context) {
  ...
  if (screen === "home") {
    lines.push(
      'Home is the starting hub. Use the cards: **Create a new course**, **Edit my courses**, **Browse Community**, or **Live class insights**.'
    );
  }

  if (screen === "dashboard") {
    lines.push('Live class insights has no "New course" button. Send the teacher to Home or My courses first.');
  }
...
function buildAssistantSystemPrompt(context) {
  ...
  return (
    ...
    "Home is the CMS starting page with task shortcuts. Live class insights is in the sidebar and on Home. " +
